...
namespace SharpICTCLAS
{
   public sealed class Utility
   {
      ...
      public static string GetPOSString(int nPOS)
      {
         ...
         sPOSRet = sPOSRet.Replace('\0', ' ');
         return sPOSRet;
      }

       public static string GetPOSString(int nPOS, nPosLevel nlevel)
       {
           return GetPOSString(nPOS).Substring(0, (int)nlevel);
       }
      #endregion
      ...
      {
            int temp = (Convert.ToInt32(b1) - 176) * 94 + (Convert.ToInt32(b2) - 161);
